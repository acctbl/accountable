...
type infisicalAPI interface {
	...
	Read(context.Context, string, SecretRef) ([]byte, error)
}
...
func (s *InfisicalSecretSource) ResolveBatch(ctx context.Context, refs []SecretRef) (map[SecretRef]SecretValue, error) {
	if len(refs) == 0 {
		return map[SecretRef]SecretValue{}, nil
	}
	seen := make(map[SecretRef]struct{}, len(refs))
...
func (a *infisicalHTTPAPI) Read(ctx context.Context, token string, ref SecretRef) ([]byte, error) {
